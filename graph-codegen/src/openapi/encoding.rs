...
pub struct Encoding {
    ...
    /// type: for object - application/json; for array – the default is defined based on the inner
    /// type; for all other cases the default is application/octet-stream. The value can be a
    /// specific media type (e.g. application/json), a wildcard media type (e.g. image/*), or a
    ...
    /// media type is not application/x-www-form-urlencoded or multipart/form-data. If a value is
    /// explicitly defined, then the value of contentType (implicit or explicit) SHALL be ignored.
    #[serde(skip_serializing_if = "Option::is_none")]
