    ...
    D: Deserializer<'de>,
{
    ...
    let map = value.as_object().ok_or_else(|| D::Error::custom(&format!("Expected struct matching either `Parameter` or `Reference`. Found the following serde_json::Value:\n{:#?}\n", value)))?;
...
{
    let value: serde_json::Value = Deserialize::deserialize(deserializer)?;
    let map = value.as_object().ok_or_else(|| D::Error::custom(&format!("Expected struct matching either `Map<String, T>` or `Map<String, Reference>`. Found the following serde_json::Value:\n{:#?}\n", value)))?;
    let mut either_map = HashMap::new();

    for (key, inner_value) in map {
        let inner_map = inner_value.as_object().ok_or_else(|| D::Error::custom(&format!("Expected struct matching either `T` or `Reference`. Found the following serde_json::Value:\n{:#?}\n", value)))?;
    ...
    D: Deserializer<'de>,
{
    ...
    for v in arr {
        let map = v.as_object().ok_or_else(|| D::Error::custom(&format!("Expected struct matching either `T` or `Reference`. Found the following serde_json::Value:\n{:#?}\n", v)))?;
