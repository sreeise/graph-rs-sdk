...
// Bytes and BytesMut from the bytes crate, files, Path/PathBuf, etc.
...
// Requires bytes = { version = "1.4.0", features = ["serde"] }
async fn upload_bytes() -> GraphResult<()> {
    ...
    let mut iter = response.into_upload_session(bytes).await.unwrap();
